...
multi_gpu=1
export DATASET_NAME="toml/checkpoint/avatar_768_1024_finetuning.toml"   #<数据准备时创建的.toml文件>
export sample_prompts="toml/checkpoint/avatar_768_1024_finetuning_sample_prompts.txt"
...
python -m accelerate.commands.launch ${launchArgs[@]} --num_cpu_threads_per_process=8 "sd-scripts/fine_tune.py" \
    --pretrained_model_name_or_path="/appdata/finetuning/sd-models/yqRealistic_ft_v10-step00040000.safetensors"  \
    --dataset_config=$DATASET_NAME \
    --output_dir="/appdata/finetuning/output/finetuning_avatar/v14"  \
    --output_name="yqAvatar_ft_768_1024_v14"  \
