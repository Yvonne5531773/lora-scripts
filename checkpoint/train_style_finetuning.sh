...
multi_gpu=1
export DATASET_NAME="toml/checkpoint/style_finetuning.toml"   #<数据准备时创建的.toml文件>
export sample_prompts="toml/checkpoint/style_finetuning_sample_prompts.txt"
...
#launchArgs+=("--config_file huggingface/accelerate/multi_machines_config.yaml")
...
python -m accelerate.commands.launch ${launchArgs[@]} --num_cpu_threads_per_process=8 "sd-scripts/fine_tune.py" \
    ...
    --output_dir="/appdata/finetuning/output/finetuning_model/v11"  \
    --output_name="yqRealistic_ft_v11"  \
