...
train_data_dir="/appdata/finetuning/train/zjz_blue"              # train dataset path | 训练数据集路径
...
network_dim=64                 # network dim (Rank秩值) | 常用 4~128，不是越大越好。主要影响的就是差异信息量，r值为信息量的系数，所以r值约小，微调的信息量也就越小，其模型效果约接近底模，拟合效果越差。所以随着r值的增加，模型的记忆力也会随之增加，带来的副作用是训练时间、模型大小也会随之增加，但过大的r值可能导致过多的无用信息，容易导致模型过拟合，尤其是对于我们这种单一主体的训练。所以你要根据你训练的目标来制定数值，不过可以肯定的是r值低于8-16时，欠拟合的可能性很大，一般使用32。
...
output_name="zjz_blue_majic_576_768"           # output model name | 模型保存名称
