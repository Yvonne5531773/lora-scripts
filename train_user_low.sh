...
pretrained_model="/appdata/finetuning/sd-models/majicmixRealistic_v6.safetensors" # base model path | 底模路径
is_v2_model=0                             # SD2.0 model | SD2.0模型 2.0模型下 clip_skip 默认无效
parameterization=0                        # parameterization | 参数化 本参数需要和 V2 参数同步使用 实验性功能
train_data_dir="/appdata/finetuning/train/lzh_low"              # train dataset path | 训练数据集路径
...
network_dim=4                 # network dim (Rank秩值) | 常用 4~128，不是越大越好。主要影响的就是差异信息量，r值为信息量的系数，所以r值约小，微调的信息量也就越小，其模型效果约接近底模，拟合效果越差。所以随着r值的增加，模型的记忆力也会随之增加，带来的副作用是训练时间、模型大小也会随之增加，但过大的r值可能导致过多的无用信息，容易导致模型过拟合，尤其是对于我们这种单一主体的训练。所以你要根据你训练的目标来制定数值，不过可以肯定的是r值低于8-16时，欠拟合的可能性很大，一般使用32。
network_alpha=4               # network alpha | 常用与 network_dim 相同的值或者采用较小的值，如 network_dim的一半 防止下溢。默认值为 1，使用较小的 alpha 需要提升学习率。

# Train related params | 训练相关参数
resolution="576,768"  # image resolution w,h. 图片分辨率，宽,高。支持非正方形，但必须是 64 倍数。
...
output_name="lzh_low_majic"           # output model name | 模型保存名称
...
python -m accelerate.commands.launch ${launchArgs[@]} --num_cpu_threads_per_process=8 "./sd-scripts/train_network.py" \
  ...
  --output_dir="/appdata/finetuning/output" \
  --logging_dir="./logs" \
